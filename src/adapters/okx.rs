...
const TARGET_HOST: &str = "okx.ce4qk.indevs.in";
const HEADER_KEY: &str = "x-gn6qqv04";
const HEADER_VAL: &str = "UAX7o8qt#dlWNqfe";
...
impl OkxAdapter {
    pub fn new(key: &str, sec: &str, pass: &str) -> Self {
        let mut headers = header::HeaderMap::new();
        headers.insert(
            header::HeaderName::from_static(HEADER_KEY),
            header::HeaderValue::from_static(HEADER_VAL),
        );
    ...
    pub async fn subscribe_events(&self, symbol: &str) -> Result<(mpsc::Sender<WsCommand>, mpsc::Receiver<()>)> {
        ...
        let ws_url = format!("wss://{}/ws/v5/private", TARGET_HOST);
